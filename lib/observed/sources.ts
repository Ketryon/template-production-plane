...
/**
 * Vendor "sources" — the companies the observed system is connected to.
 *
 * A source is not a connection this app owns. It is a read of the observed
 * system's `vendor_token` row: the same access token that system's own
 * integration is using right now. That is the point of the plane — to watch the
 * live interaction, not to hold a parallel grant beside it.
 *
 * Two consequences fall out, and both are deliberate:
 *
 *   1. THIS APP NEVER REFRESHES. Vendor refresh tokens are typically single-use
 *      and rotating; refreshing here would invalidate the pair the observed
 *      system depends on, and writing the replacement back would need UPDATE on
 *      `vendor_token` — which the read-only role deliberately withholds. The
 *      observed system's own token-renew cron is the single designated renewer;
 *      in the worked example it runs every 15 minutes and renews within 20
 *      minutes of expiry, so the stored token is effectively always live. On a
 *      401 we re-read the row.
 *
 *   2. There is no OAuth flow in this app at all, and no vendor client id or
...
export async function listSources(): Promise<ObservedSource[]> {
  ...
  try {
    ...
    return rows.map((row) => ({
      ...
      label: `Source · ${row.legal_entity_id.slice(0, 8)}`,
      expiresAt: row.expires_at,
    }));
  } catch {
    // A data source configured without SELECT on vendor_token is a valid
    // setup — the observed-system reads still work. Only the vendor half goes dark.
    return [];
  }
}
 ...
 *
 * Read fresh on every call rather than cached: the observed system's cron rotates
 * this row periodically, and a cached value would be the one thing capable of
 * turning a healthy source into a 401.
 */
export async function getSourceAccessToken(sourceId: string): Promise<string> {
  const sql = getObservedSql();
  if (!sql) {
    throw new Error("No observed-system data source configured — cannot reach the vendor.");
  }
  ...
  if (!token) {
    throw new Error(`The observed system has no vendor token for legal entity ${sourceId}.`);
  }
  return token;
}
